...
export async function sshUpdateRV(ips, users, vencimiento, LogIn, enable) {
    let overallResults = []; // Para almacenar los resultados de todas las IPs
    ...
    const getSSHKey = async (ip) => {
        try {
            const result = await wixData.query("VPSservice").eq("ip", ip).find();
            if (result.items.length === 0) {
                throw new Error(`Error, no se encontró la clave para la IP: ${ip}.`);
            }
            return result.items[0].clave;
        } catch (error) {
            throw new Error(`Error al obtener la clave SSH: ${error.message} en la IP: ${ip}`);
        }
    };

    const delay = (ms) => new Promise(resolve => setTimeout(resolve, ms));

    const executeCommand = (conn, command, retries = 1, delayTime = 1000, timeout = 4000) => {
        return new Promise((resolve, reject) => {
            const attempt = (retryCount) => {
    ...
    const connectSSH = (conn, host, username, password, timeout = 8000) => {
        return new Promise((resolve, reject) => {
            const timeoutId = setTimeout(() => {
                reject(new Error(`Error: Conexión SSH ha superado el tiempo de espera de 4 segundos. en ${host}`));
                conn.end(); // Cierra la conexión si el tiempo se agota
            }, timeout);

            conn.on('ready', () => {
                clearTimeout(timeoutId); // Limpia el temporizador si la conexión es exitosa
                resolve();
            }).on('error', (err) => {
                clearTimeout(timeoutId); // Limpia el temporizador si hay un error
                reject(new Error(`Error de conexión SSH: ${err.message} en ${host}`));
            }).connect({
                host: host,
                port: 22,
                username: username,
    ...
    async function checkUserExists(conn, user) {
        const checkUserCommand = `id -u ${user}`;
        try {
            await executeCommand(conn, checkUserCommand, 1);
            return true;
        } catch (err) {
            const stderrMessage = err.message.toLowerCase();
            if (stderrMessage.includes('no such user') || stderrMessage.includes('no such file or directory') || stderrMessage.includes('id:') && stderrMessage.includes('no such')) {
    ...
    // Procesar todas las IPs simultáneamente con Promise.all
    const promises = ips.map(async (ip) => {
        ...
        if (ip !== "0.0.0.0") {
            try {
                ...
                const userPromises = users.map(async (userObj) => {
                    const user = userObj.user;
                    const password = userObj.password;
                    let createUserCommand = "";

                    const userExists = await checkUserExists(conn, user);
                    if (!userExists) {
                        createUserCommand = `sudo useradd -m -s /bin/false ${user}`;
                    }
                    if (!LogIn) { LogIn = 3; }

                    const enableUserCmd = enable === true ? `sudo passwd -u ${user}` : (enable === false ? `sudo passwd -l ${user}` : "");
                    const setPasswordCommand = `echo "${user}:${password}" | sudo chpasswd`;
                    const setSessionsLimitCommand = `echo "${user} hard maxlogins ${LogIn}" | sudo tee -a /etc/security/limits.conf`;
                    const setExpirationDateCommand = `sudo chage -E "$(date -d '+${vencimiento} days' '+%Y-%m-%d')" ${user}`;
                    const combinedCommand = [createUserCommand, setPasswordCommand, setSessionsLimitCommand, setExpirationDateCommand, enableUserCmd].filter(Boolean).join(' && ');

                    try {
                        const result = await executeCommand(conn, combinedCommand);
                        console.log(`Usuario ${user} en ${ip} guardado con éxito:`, result);
                        overallResults.push({ ip, user, result: String(result), toString: function () { return `___________\nUbicación: ${this.ip}\nUsuario/HWID: ${this.user}\nResultado: ${this.result}\n` } });
                    } catch (error) {
                        console.error(`Error al guardar el usuario ${user} en ${ip}: ${error.message || String(error)}`);
                        overallResults.push({ ip, user, error: String(error.message || error), toString: function () { return `___________\nUbicación: ${this.ip}\nUsuario/HWID: ${this.user}\nError: ${this.error}\n` } });
                    }
                });
                ...
                console.error(`Error en la conexión SSH o comandos para ${ip}: ${error.message || String(error)}`);
                overallResults.push({ ip, error: String(error.message || error), toString: function () { return `___________\nUbicación: ${this.ip}\nError: ${this.error}\n` } });
            } finally {
